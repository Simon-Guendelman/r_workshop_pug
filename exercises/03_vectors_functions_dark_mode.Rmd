...
```{r}
load("~/Desktop/R_course/r_workshop_pug/exercises/data_sets/hotdog.RData")
...
summary(hotdog_2018)
```
3.5) Use `head()` and `tail()` on either of the vectors. What do these functions do? Head fx the beggining elements, tail fx the last elements
...
Here for permanent removing
```{r}
numbers <- numbers[-c(3,4)] 
numbers
```
...
```{r}
summary(c(1, 20, NA, 12, 3))
```

5.4) `table()`

```{r}
table(c(1, 20, NA, 12, 3))
```
