...
nerd_red %>% 
  ...
  geom_point() +
  labs(
    title = "Level of Nerdiness across age, by gender",
    subtitle = "All good",
    caption = "Really",
...
nerd_red %>% 
  ...
  labs(y = "sum score nerdiness questionnaire") +
  theme(legend.position = "top") + # is overwritten by the theme
  theme_dark() # overwrites the previous setting
```
...
4.6) Try the arguments `size` and `alpha` (transparency, goes from 0 to 1) outside of `aes()`. If you want to, you can use other geoms and try out how these arguments affect them.

```{r}
nerd_red %>% 
  ggplot(aes(x = age, y = nerd_score)) +
  geom_point(colour = "dark green", size = 5, alpha = .5, colour = "#737322")
```

```{r}
nerd_red %>% 
  ggplot(aes(x = age, y = nerd_score)) +
  geom_point(colour = "#737322", size = 5, alpha = .5)
```
